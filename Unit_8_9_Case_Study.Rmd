...
#beers = read.csv("D:\\MSDS\\DS 6306\\MSDS_6306_Doing-Data-Science\\Unit 8 and 9 Case Study 1\\beers.csv",header = TRUE)
#Breweries = read.csv("D:\\MSDS\\DS 6306\\MSDS_6306_Doing-Data-Science\\Unit 8 and 9 Case Study 1\\Breweries.csv",header = TRUE)
...
We felt that just looking at the median values did not tell the whole story and there was other statistical measures worth looking at and comparing by state, thus we decided to also look at a box plot of the same data.

```{r, fig.width=16, fig.height=8}
library(plyr)
#ABV
merged_clean = merged %>% filter(!is.na(Beer_ID) & !is.na(ABV))

meds <- ddply(merged_clean, .(State), summarise, med = median(ABV))

merged_clean %>% ggplot(aes(x=reorder(State,ABV,median), y=ABV)) + geom_boxplot() +
  labs(title="ABV by State", x ="State",y="ABV")+
  geom_text(data = meds, aes(x = State, y = med, label = med),
            size = 3, vjust = -1.5) 
#IBU
merged_clean2 = merged %>% dplyr::filter(!is.na(Beer_ID) & !is.na(IBU))

meds <- ddply(merged_clean2, .(State), summarise, med = median(IBU))

merged_clean2 %>% ggplot(aes(x=reorder(State,IBU,median), y=IBU)) + geom_boxplot() +
